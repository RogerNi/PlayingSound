...
class SoundSyn:
    ...
    def set_cutpoint(self, cutpoint):
        self.cutpoint = cutpoint
        
    def set_sustainLevel(self, l):
        self.sustainLevel = l
        
    def make_wave(self):
#        self.wave = readTrackFile(self.filename, self.scale, self.partialAmp, self.partialRatio, self.bpm)
        self.wave = readTrackFile(self)
        return self.wave


def readTrackFile(ss):
    f = open(ss.filename,"r")
    llist = f.readlines()
...
#    d = 1
    ...
    wave = genTrack(trackList, ss)
#        wave.scale(d)
    
    for track in llist[:-1]:
        trackList = track.split(" ")
        w = genTrack(trackList, ss)
#            w.scale(d)
        wave = wave + w
...
def genTrack(trackList, ss):
    ...
    w = genTrackByNoteList(noteList, timeList, ss)
    return(w)

def makeWindow(length, cutpoint, ratio, susLevel):
    window = [0] * length
    
    c1 = round(length * cutpoint[0] * ratio)
    if (c1 > 200):
        c1 = 200
    c2 = round(length * cutpoint[1] * ratio)
    c3 = round(length * cutpoint[2] * ratio)
    
    for i in range(c1):
        window[i] = i / c1
    for i in range(c1, c2):
        window[i] = (c2 - i) * (1-susLevel) / (c2 - c1) + susLevel
    for i in range(c2, c3):
        window[i] = susLevel
    for i in range(c3, length):
        window[i] = (length - i) * susLevel / (length - c3)
    return(window)

# 1/3 / (c1 - c2) = (wi - 2/3) / (i - c2) 
# 2/3 / (c3 - length)  = (wi) / (i - c3)

def makeWindow_orig(length, cutpoint):
    window = [0] * length
    '''
...
def genToneByNote(note, dur, ss):
    f = ss.scale[noteToIndex(note)]
    '''
    if scale == 1:
...
#        f = etScaleAS[noteToIndex(note)]    
    '''
    w = genToneByFreq(f,dur,ss)
    return(w)
   

#def genChordByNote(noteList, dur, scale, partialAmp, partialRatio):
def genChordByNote(noteList, dur, ss):
#    d = 1/len(noteList)
    waveList = list()
    for note in noteList:
        w = genToneByNote(note,dur,ss)
#        w.scale(d)
        waveList.append(w)
...
def genTrackByNoteList(noteList, timeList, ss):
    '''    
    if len(noteList[0]) <= 3:  
    ...
    for i in range(l):
        if len(noteList[i]) <= 3:
            ...
            wave = wave | genChordByNote(nlist, timeList[i]*d, ss)
